...
OUT = "PantryPilot_System_Testing_Report_Zixuan_Liang.docx"
...
def build_report():
    ...
    add_para(
        doc,
        "This report documents the system-level testing approach for PantryPilot across its current project versions. The report includes the development and test environment, setup instructions, test-case selection methodology, detailed system test procedures, expected results, observed actual results, and final outcome. The tests are intentionally system-level: they exercise externally meaningful user workflows such as release configuration, quality gates, grocery-list generation, allergen blocking, safe recipe approval, and pantry-expiry boundary handling.",
    )
    add_para(
        doc,
        "All six executed system scenarios passed. The local test execution used Python 3.11.15 on macOS 26.5.1. The configured CI environment uses GitHub Actions on ubuntu-latest, Python 3.12, and pytest 8.2.2. Because pytest was not installed in the local runtime, the same test functions were also executed through a lightweight system-test runner to capture actual output for this report.",
    )
    ...
    rows = [
        ...
        ("Application repository", "PantryPilot_CM_Generic_Files", "Source package under test."),
    ...
    add_para(
        doc,
        "The system test cases were selected from the highest-value user-visible behavior and the highest-risk requirements in the PantryPilot project. The goal was not to test every function in isolation; it was to confirm that each release behaves correctly from a system perspective. The test set therefore includes configuration loading, release identity, quality-gate configuration, grocery aggregation, allergen safety filtering, safe-output acceptance, and pantry expiry boundary behavior.",
    )
    add_para(
        doc,
        "The selected cases provide adequate coverage for the current project scope because they exercise every major behavior present in the submitted application baseline and directly connect to the risk-management report. The allergen and expiry cases cover safety-critical negative and boundary behavior; the grocery case covers user-visible output correctness; the configuration and quality-gate cases verify release readiness and testability. V2 features such as nutrition optimization and full pantry-aware planning are listed as future scope because the current application configuration keeps them disabled.",
    )

    doc.add_page_break()
    add_heading(doc, "System Test Plan and Results")
    add_test_overview_table(doc)
    add_test_case_detail_tables(doc, table_start=4)

    add_heading(doc, "Execution Summary")
    add_caption(doc, "Table 10", "System test outcome summary")
    table = doc.add_table(rows=1, cols=4)
    ...
    add_para(
        doc,
        "The current system tests cover the implemented release baseline. They do not claim that unavailable future features have been fully tested. Nutrition optimization, AI-generated recipes, and full pantry-aware planning remain future V2 scope in the project proposal and should receive additional end-to-end tests when implemented. Future tests should include infeasible nutrition targets, recorded LLM responses, recipe API fixture replay, multi-profile households, and full weekly meal-plan export.",
    )

    add_heading(doc, "Conclusion")
    add_para(
        doc,
        "The PantryPilot system testing process demonstrates that the submitted application versions can be installed, identified, configured, and exercised at the system level. The executed tests produced the expected outputs with no observed errors. The selected cases provide adequate coverage for the current project baseline because they cover the implemented release behavior and the most important user-facing and risk-driven scenarios.",
    )
